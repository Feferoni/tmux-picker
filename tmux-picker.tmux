...
"(($CS|^|$FILE_START_CHARS)$FILE_CHARS+\\.$FILE_CHARS+:)" # anything that looks like file/file path but not too short
"(()(https?://|git@|git://|ssh://|ftp://|file:///)[[:alnum:]?=%/_.:,;~@!#$&)(*+-]*)" # URLs
"((^|$FILE_START_CHARS)($FILE_CHARS*/$FILE_CHARS+(?::[0-9]+(?:\:[0-9]+)?)?))"
)
...
BLACKLIST=(
"(deleted|modified|renamed|main|copied|master|mkdir|[Cc]hanges|update|updated|committed|commit|working|discard|directory|staged|add/rm|checkout)"
)
